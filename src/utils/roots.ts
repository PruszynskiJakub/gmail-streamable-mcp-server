...
export async function requestRoots(server: McpServer): Promise<Root[]> {
  ...
  try {
    type LowLevel = {
      server?: LowLevel;
      request?: (r: unknown, s: unknown) => Promise<unknown>;
...
export function clientSupportsRootsListChanged(server: McpServer): boolean {
  try {
    type LowLevel = {
      server?: LowLevel;
      getClientCapabilities?: () => Record<string, { listChanged?: unknown }>;
    };
    const lowLevel = ((server as unknown as LowLevel).server ?? server) as LowLevel;
    const clientCapabilities = lowLevel.getClientCapabilities?.() ?? {};
    return Boolean(
      (clientCapabilities.roots as { listChanged?: unknown } | undefined)?.listChanged,
    );
  } catch {
    return false;
  }
