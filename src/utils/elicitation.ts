...
export function clientSupportsFormElicitation(server: McpServer): boolean {
  try {
    ...
    const lowLevel = ((server as unknown as LowLevel).server ?? server) as LowLevel;
...
export function clientSupportsUrlElicitation(server: McpServer): boolean {
  try {
    type LowLevel = {
      server?: LowLevel;
      getClientCapabilities?: () => Record<string, { url?: unknown }>;
    };
    const lowLevel = ((server as unknown as LowLevel).server ?? server) as LowLevel;
    const clientCapabilities = lowLevel.getClientCapabilities?.() ?? {};
    return Boolean(
      (clientCapabilities.elicitation as { url?: unknown } | undefined)?.url,
    );
  } catch {
    return false;
  }
...
export async function elicitForm(
  ...
  try {
    type LowLevel = {
      server?: LowLevel;
      request?: (r: unknown, s: unknown) => Promise<ElicitResult>;
    };
    const lowLevel = ((server as unknown as LowLevel).server ?? server) as LowLevel;
...
export async function notifyElicitationComplete(
  ...
  try {
    type LowLevel = { server?: LowLevel; notification?: (p: unknown) => Promise<void> };
    const lowLevel = ((server as unknown as LowLevel).server ?? server) as LowLevel;
